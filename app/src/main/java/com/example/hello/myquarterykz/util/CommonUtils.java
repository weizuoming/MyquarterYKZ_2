...
public class CommonUtils {
    ...
    public static void runOnUIThread(Runnable runable) {
        //先判断当前属于子线程还是主线程
        if (android.os.Process.myTid() == WieApplication.getMainThreadId()) {
            runable.run();
        } else {
            //子线程
            WieApplication.getAppHanler().post(runable);
        }
    }
    ...
    public static void saveString(String flag, String str) {
        if (sharedPreferences == null) {
            sharedPreferences = WieApplication.getAppContext().getSharedPreferences(TAG, WieApplication.getAppContext().MODE_PRIVATE);
        }
